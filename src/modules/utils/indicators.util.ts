...
import { Candle } from '../strategy/core/strategy.interface';
...
export function getATR(candles: number[][], period: number): number[] {
  ...
  for (let i = 0; i < trs.length; i++) {
    if (i < period - 1) {
      atr.push(NaN); // недостаточно данных
    } else if (i === period - 1) {
      // Первое значение ATR — простое среднее
      const sum = trs.slice(0, period).reduce((a, b) => a + b, 0);
      atr.push(sum / period);
    } else {
      // Остальные значения — EMA-подобное сглаживание
...
export function getVWAP(candles: number[][]): number[] {
  const vwap: number[] = [];
  let cumulativeTPV = 0;
...
export function getHigherTimeframeCandles(candles: Candle[], targetTimeframe: string): Candle[] {
  ...
  const currentTimeframe = candles[1][0] - candles[0][0];
  ...
  const ratio = targetMinutes / (currentTimeframe / 60000);
  ...
  for (const candle of candles) {
    ...
    // Обновляем high/low
    temp[2] = Math.max(temp[2], high);
    temp[3] = Math.min(temp[3], low);
    temp[4] = close; // Close последней свечи
    temp[5] += volume; // Суммируем объем

    count++;

    // Формируем новую свечу при достижении интервала
    if (count >= ratio) {
      higherTF.push(temp);
      temp = null;
    }
  ...
  // Добавляем последнюю неполную свечу
  if (temp) {
    higherTF.push(temp);
  }
...
function findNearestPeak(peaks: { index: number; value: number }[], targetIndex: number) {
  ...
  return peaks.reduce((nearest, peak) =>
    Math.abs(peak.index - targetIndex) < Math.abs(nearest.index - targetIndex) ? peak : nearest
  );
}
  ...
  swingPoints.forEach((point, i) => {
    if (point.type === 'high') {
      ...
      if (i > 0 && i < swingPoints.length - 1) {
        const isSignificant =
          point.value > swingPoints[i - 1].value && point.value > swingPoints[i + 1].value;
...
export function findOrderBlocks(candles: Candle[]): { bullish: Candle[]; bearish: Candle[] } {
  ...
  for (let i = 2; i < candles.length - 1; i++) {
    ...
    if (
      current[4] > current[1] &&
      current[4] - current[3] > 2 * (current[2] - current[4]) &&
      next[4] > next[1] &&
      next[4] > current[4]
    ) {
    ...
    if (
      current[4] < current[1] &&
      current[2] - current[4] > 2 * (current[4] - current[3]) &&
      next[4] < next[1] &&
      next[4] < current[4]
    ) {
...
export function calculateRiskReward(
  ...
  if (!takeProfitLevel) {
    return signal === 'buy'
      ? (entryPrice + 3 * risk - entryPrice) / risk
      : (entryPrice - (entryPrice - 3 * risk)) / risk;
  }
